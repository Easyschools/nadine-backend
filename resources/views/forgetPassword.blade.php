        ...
        <div class='row' style='height: 100%;'>
            <div class='col-auto m-auto'>
                <section class='form-box'>
                    <h1>Welcome {{ app('request')->input('name') }}</h1>
                    ...
                    <form method='post' action="{{route('reset')}}">
                        ...
                        @endif
                        <input name="email" value='{{app('request')->input('email')}}' type='hidden'>
                        @if($errors->has('email'))
                            <div style="color: darkred">{{ $errors->first('email') }}</div>
                        @endif
                        <input name="token" value='{{app('request')->input('token')}}' type='hidden'>
                        @if($errors->has('token'))
                            <div style="color: darkred">{{ $errors->first('token') }}</div>
                        @endif
                        <input name="password" placeholder='Type New Password' title='Type New Password' type='password'>
                        @if($errors->has('password'))
                            <div style="color: darkred">{{ $errors->first('password') }}</div>
                        @endif
                        <input name="password_confirmation" placeholder='Retype New Password' title='Retype New Password' type='password'>
                        @if($errors->has('password_confirmation'))
                            <div style="color: darkred">{{ $errors->first('password_confirmation') }}</div>
                        @endif
